...
let update_index :
  ...
  let ok_exn =
    ...
    function
    | OK -> ()
    | e -> failwith (sprintf "Sqlite error: %s" (to_string e))
  in
  let done_exn =
    let open Sqlite3.Rc in
    function
  ...
  fun ~pkg db ids ->
    let url id =
      ...
      match Url.from_identifier ~get_package:(fun _ -> pkg) ~stop_before:false id with
      | Ok { page; anchor; kind } ->
...
let add_anchors : pkg:string -> Fpath.t -> ('a Paths.Identifier.any * string) list -> unit =
  fun ~pkg docu_dir ids ->
    List.fold_left (fun m (id, type_) ->
        let open DocOckHtml in
        let url =
          Url.from_identifier ~get_package:(fun _ -> pkg) ~stop_before:false id
        in
        let Url.({ page; anchor }) = match url with
          | Ok x -> x
          | Error e -> failwith (Url.Error.to_string e)
        in
        let file =
        ...
        in
        if anchor = "" || not (OS.File.exists file |> ok_exn) then m else
